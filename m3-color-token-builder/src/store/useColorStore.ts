...
import {
  spacing as initialSpacing,
  borderRadius as initialBorderRadius,
} from '../design-system/tokens/spacing';
import {
  shadows as initialShadows,
  elevation as initialElevation,
...
export const useColorStore = create<ColorState>((set) => {
  ...
  const initLightRoles = buildRolesList(
    DEFAULT_LIGHT_ROLES,
    'light',
    DEFAULT_KEY_COLORS,
    initPalettes,
  );
  const initDarkRoles = buildRolesList(
    DEFAULT_DARK_ROLES,
    'dark',
    DEFAULT_KEY_COLORS,
    initPalettes,
  ...
  return {
    ...
    history: (() => {
      try {
        return JSON.parse(localStorage.getItem('matisse_history') || '[]');
      } catch {
        return [];
      }
    })(),
    ...
    // Design Token Actions
    updateTypographyFamily: (key, value) =>
      set((state) => ({
        typography: {
          ...state.typography,
    ...
    updateTypographySize: (key, value) =>
      set((state) => ({
        typography: {
          ...state.typography,
    ...
    updateTypographyWeight: (key, value) =>
      set((state) => ({
        typography: {
          ...state.typography,
    ...
    updateTypographyLineHeight: (key, value) =>
      set((state) => ({
        typography: {
          ...state.typography,
    ...
    updateTypographyLetterSpacing: (key, value) =>
      set((state) => ({
        typography: {
          ...state.typography,
    ...
    updateSpacingValue: (key, value) =>
      set((state) => ({
        spacing: { ...state.spacing, [key]: value },
      })),

    updateSpacingUnit: (baseUnitPx) =>
      set((state) => {
    ...
    updateBorderRadiusValue: (key, value) =>
      set((state) => ({
        borderRadius: { ...state.borderRadius, [key]: value },
      })),

    updateShadowValue: (key, value) =>
      set((state) => ({
        shadows: { ...state.shadows, [key]: value },
      })),

    updateElevationValue: (key, value) =>
      set((state) => ({
        elevation: { ...state.elevation, [key]: value },
      })),

    updateShadowGlow: (key, value) =>
      set((state) => ({
        shadows: {
          ...state.shadows,
